...
import { storage, Company, User, State, District, Block, School as SchoolType, Trade, Trainer } from '@/lib/storage';
...
const CompanyDashboard: React.FC<CompanyDashboardProps> = ({ user, onLogout }) => {
  ...
  const filteredData = () => {
    ...
    if (filters.stateId) {
      ...
      filteredDistricts = districts.filter(d => d.stateId === filters.stateId);
      filteredBlocks = blocks.filter(b => filteredDistricts.some(d => d.id === b.districtId));
      filteredSchools = schools.filter(s => s.stateId === filters.stateId);
    }

    if (filters.districtId) {
      filteredDistricts = filteredDistricts.filter(d => d.id === filters.districtId);
      filteredBlocks = blocks.filter(b => b.districtId === filters.districtId);
      filteredSchools = schools.filter(s => s.districtId === filters.districtId);
    }
  ...
  const handleAddState = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newState.name || !user.companyId) return;
            ...
            {statistics.stateWiseStats?.length > 0 && (
              <Card className="backdrop-blur-sm bg-card/80">
                <CardHeader>
                  ...
                  <CardDescription>School and infrastructure breakdown by state</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {statistics.stateWiseStats.map((state: any) => (
                      <div key={state.id} className="p-4 rounded-lg border border-border/50 bg-background/50 hover:bg-background/80 transition-colors">
                        <h3 className="font-semibold text-lg mb-2">{state.name}</h3>
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div>
                            <div className="text-xl font-bold text-primary">{state.districts}</div>
                            <div className="text-xs text-muted-foreground">Districts</div>
                          </div>
                          <div>
                            <div className="text-xl font-bold text-accent-foreground">{state.blocks}</div>
                            <div className="text-xs text-muted-foreground">Blocks</div>
                          </div>
                          <div>
                            <div className="text-xl font-bold text-secondary-foreground">{state.schools}</div>
                ...
                <CardContent>
                  <form onSubmit={handleAddDistrict} className="space-y-4">
                    <div>
                      ...
                      <Select value={newDistrict.stateId} onValueChange={(value) => setNewDistrict({...newDistrict, stateId: value})}>
                    ...
                    <Button type="submit" className="w-full" disabled={!newDistrict.stateId}>
                      <Plus className="w-4 h-4 mr-2" />
                      Add District
                    </Button>
                  </form>
                ...
                <CardContent>
                  <form onSubmit={handleAddBlock} className="space-y-4">
                    <div>
                      ...
                      <Select value={newBlock.districtId} onValueChange={(value) => setNewBlock({...newBlock, districtId: value})}>
                        <SelectTrigger className="mt-1">
                    ...
                    <Button type="submit" className="w-full" disabled={!newBlock.districtId}>
                      <Plus className="w-4 h-4 mr-2" />
                      Add Block
                    </Button>
                  </form>
                                  ...
                                  <div key={trade.id} className="p-2 rounded bg-background/50">
                                    ...
                                    {tradeTrainers.length > 0 ? (
                                      <div className="text-xs text-muted-foreground">
                                        Trainers: {tradeTrainers.map(t => t.name).join(', ')}
                                      </div>
                                    ) : (
